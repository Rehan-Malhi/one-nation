...
export default function TestimonialsAuto() {
  ...
  return (
    <FadeInOnScroll yOffset={30} duration={0.8}>
          ...
          >
            {items.map((t, i) => (
              <article
                key={i}
                data-card
                className="shrink-0 bg-white rounded-sm border border-black/10 p-7 sm:p-8 shadow-[0_20px_70px_-55px_rgba(0,0,0,0.5)] hover:shadow-[0_25px_80px_-50px_rgba(0,0,0,0.6)] transition-shadow duration-300 min-h-[260px] w-full md:w-[calc((100%-48px)/3)]"
              >
                {/* Header */}
                <div className="flex items-center gap-3">
                  ...
                  <h4 className="text-sm text-black font-bold">{t.name}</h4>
                </div>
                ...
                <div className="mt-8 flex justify-between items-end">
                  ...
                  <span className="text-sm font-semibold underline underline-offset-4 transition-colors duration-200 hover:text-black/80">
                    Read More
                  </span>
                </div>
              </article>
